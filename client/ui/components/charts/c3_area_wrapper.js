...
export class C3AreaWrapper {
   ...
   */
  parseData(data) {
    debug && console.log(Util.timestamp(), 'C3AreaWrapper.parseData:', this.containerId, data && data.length);
    let self = this,
      map = {};

    //console.log(Util.timestamp(), 'C3AreaWrapper.parseData:', data);
    self.columns = data;
    return
    ...
    data.forEach((row) => {
      ...
      if (columnKey !== undefined) {
        ...
        if (self.config.aggregation === 'count') {
          ...
          console.error('C3AreaWrapper.parseData unknown aggregation:', self.config.aggregation);
        }
      }
    });
