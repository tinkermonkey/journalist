...
Template.WeeklySupportReport.helpers({
  ...
  fixVersionList () {
    return this.document.fields.fixVersions.map((version) => {
      return version.name
    }).join(', ')
  },
  newTicketsCount () {
  ...
  installationsAffected () {
    let project        = this,
        ...
        data           = _.flatten(supportTickets.map((item) => {
          return item.document.fields.installationsaffected
        }));
  ...
  supportAreas () {
    let project        = this,
        ...
        supportTickets = Template.instance().supportTickets.get() || [],
        data           = _.flatten(supportTickets.map((item) => {
          return item.document.fields.supportarea
        }));
  ...
  supportTicketAge () {
    let project          = this,
        ...
        withFixesAges    = supportTickets.filter((item) => {
            ...
            .map((item) => {
              return moment().diff(moment(item.dateCreated), 'days')
            }),
        ...
        withFixesData    = segments.map((floor, i) => {
          ...
          return {
            ...
            value: withFixesAges.filter((dataPoint) => {
              return dataPoint >= floor && dataPoint < ceiling
            })
          }
        }),
        withoutFixesData = segments.map((floor, i) => {
          ...
          return {
            title: i < segments.length - 1 ? floor.toString() + ' - ' + ceiling.toString() : floor.toString() + '+',
            value: withoutFixesAges.filter((dataPoint) => {
              return dataPoint >= floor && dataPoint < ceiling
            })
          }
        });
    ...
    return {
      ...
      config  : {
        ...
        chart         : {
          axis  : {
            x: {
              type      : 'category',
              categories: withFixesData.map((d) => {
                return d.title + ' days'
              })
            }
          },
      ...
      data    : [
        [ 'With Fixes' ].concat(withFixesData.map((d) => {
          return d.value.length
        })),
        [ 'Without Fixes' ].concat(withoutFixesData.map((d) => {
          return d.value.length
        }))
      ]
    }
