...
Meteor.methods({
  ...
  /**
   * Add an integration status map
   * @param title
   * @param serverId
   */
  addIntegrationStatusMap (title, serverId) {
    console.log('addIntegrationStatusMap:', title, serverId);
    let user = Auth.requireAuthentication();
    ...
    if (user.isAdmin()) {
      // Insert the project percent
      IntegrationStatusMaps.insert({
        title   : title,
        serverId: serverId
      });
    } else {
      console.error('Non-admin user tried to add an integration status map:', user.username, title);
      throw new Meteor.Error(403);
    }
  },
  
  /**
   * Remove an integration status map
   * @param mapId
   */
  deleteIntegrationStatusMap (mapId) {
    console.log('deleteIntegrationStatusMap:', mapId);
    let user = Auth.requireAuthentication();
    ...
    // Validate that the current user is an administrator
    if (user.isAdmin()) {
      IntegrationStatusMaps.remove(mapId);
    } else {
      console.error('Non-admin user tried to delete an integration status map:', user.username, mapId);
      throw new Meteor.Error(403);
    }
  },
  
  /**
   * Edit an integration status map record
   * @param mapId
   * @param key
   * @param value
   */
  editIntegrationStatusMap (mapId, key, value) {
    console.log('editIntegrationStatusMap:', mapId, key);
    let user = Auth.requireAuthentication();
    ...
    // Get the status map record to make sure this is authorized
    let statusMap = IntegrationStatusMaps.findOne(mapId);
    ...
    if (user.isAdmin()) {
      if (statusMap) {
        ...
        // Update the contributor
        IntegrationStatusMaps.update(mapId, { $set: update });
      } else {
        throw new Meteor.Error(404);
      }
    } else {
      console.error('Non-admin user tried to edit an integration status map:', user.username, key, mapId);
      throw new Meteor.Error(403);
    }
  },
  
  /**
   * Add an integration status mapping to a status map
   * @param statusMapId
   */
  addIntegrationStatusMapping (statusMapId) {
    console.log('addIntegrationStatusMapping:', statusMapId);
    let user = Auth.requireAuthentication();
    
    // Validate the data is complete
    check(statusMapId, String);
    
    // Validate that the current user is an administrator
    if (user.isAdmin()) {
      // Insert the project percent
      IntegrationStatusMappings.insert({
        statusMapId: statusMapId
      });
    } else {
      console.error('Non-admin user tried to add an integration status mapping:', user.username, statusMapId);
      throw new Meteor.Error(403);
    }
  },
  
  /**
   * Remove an integration status mapping
   * @param mappingId
   */
  deleteIntegrationStatusMapping (mappingId) {
    console.log('deleteIntegrationStatusMapping:', mappingId);
    let user = Auth.requireAuthentication();
    ...
    // Validate that the current user is an administrator
    if (user.isAdmin()) {
      IntegrationStatusMappings.remove(mappingId);
    } else {
      console.error('Non-admin user tried to delete an integration status mapping:', user.username, mappingId);
      throw new Meteor.Error(403);
    }
  },
  
  /**
   * Edit an integration status mapping record
   * @param mappingId
   * @param key
   * @param value
   */
  editIntegrationStatusMapping (mappingId, key, value) {
    console.log('editIntegrationStatusMapping:', mappingId, key);
    let user = Auth.requireAuthentication();
    ...
    // Get the status mapping record to make sure this is authorized
    let statusMapping = IntegrationStatusMappings.findOne(mappingId);
    ...
    if (user.isAdmin()) {
      if (statusMapping) {
        ...
        // Update the contributor
        IntegrationStatusMappings.update(mappingId, { $set: update });
      } else {
        throw new Meteor.Error(404);
      }
    } else {
      console.error('Non-admin user tried to edit an integration status mapping:', user.username, key, mappingId);
      throw new Meteor.Error(403);
    }
  },
