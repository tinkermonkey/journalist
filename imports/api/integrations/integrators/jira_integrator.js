...
// Make the cookie store synchronous
ToughCookie.Store.synchronous = true;
...
export class JiraIntegrator extends Integrator {
  ...
  /**
   * Verify that the server is responding regardless of authentication
   */
  checkConnectivity() {
    debug && console.log('JiraIntegrator.checkConnectivity:', this.provider && this.provider.server.title);
    let self = this;

    return self.fetchData('myPermissions', 'getMyPermissions');
  }
