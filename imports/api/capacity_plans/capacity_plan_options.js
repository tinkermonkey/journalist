    ...
    if (option) {
      for (let i = 0; i < option.sprintCount; i++) {
        CapacityPlanSprints.insert({
          ...
          optionId    : option._id,
          sprintNumber: i,
          startDate   : moment(option.startDate).add(i * option.sprintLength, 'ms').startOf('isoWeek').add(1, 'days').toDate(),
          endDate     : moment(option.startDate).add((i + 1) * option.sprintLength, 'ms').startOf('isoWeek').subtract(2, 'days').toDate()
        });
      }
    }
  });
