...
CapacityPlanOptions.helpers({
  ...
  healContributorLinks (contributorId) {
    ...
    //console.log('CapacityPlanOptions.healContributorLinks sprints:', sprints);
    // Remove any links from this contributor to a different sprint than the first one
    CapacityPlanSprintLinks.find({ sourceId: contributorId, targetSprint: { $gt: sprints[ 0 ] } }).forEach((link) => {
    ...
    firstSprintBlocks.forEach((block) => {
      ...
      // Make sure there is at least one link to this block
    ...
    if (sprints.length > 1) {
      ...
      // Get the blocks for the only sprint
      blockCriteria.sprintNumber = sprints[ 0 ];
      let sprintBlocks           = CapacityPlanSprintBlocks.find(blockCriteria).fetch();
      
      sprintBlocks.forEach((block) => {
        // Remove any links from this block
        CapacityPlanSprintLinks.find({ sourceId: block._id }).forEach((link) => {
          console.info('CapacityPlanOptions.healContributorLinks removing link that jumps a sprint:', link.sourceId, link.targetId, link.autoGenerated);
          CapacityPlanSprintLinks.remove(link._id);
        });
        
        // Remove any links not from a contributor block
        block.targetLinks().forEach((link) => {
          if (link.sourceType !== CapacityPlanBlockTypes.contributor) {
            CapacityPlanSprintLinks.remove(link);
          }
        });
      })
    }
