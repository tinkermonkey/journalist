...
export class IntegrationAgent {
  ...
  constructor (integration, serviceProvider) {
    logger.info('Creating new IntegrationAgent:', integration._id);
    let self = this;
    ...
    if (!self.serviceProvider) {
      logger.error('IntegrationAgent could not find service provider:', integration.server());
    }
    ...
    Meteor.defer(() => {
      self.observer = Integrations.find({ _id: self.integration._id }).observe({
        changed (newDoc, oldDoc) {
          debug && logger.info('IntegrationAgent.observer.changed:', newDoc._id, newDoc.title);
   ...
   */
  updateQueryJobs () {
    debug && logger.info('IntegrationAgent.updateQueryJobs:', this.integration._id);
    let self             = this,
        ...
        _.keys(queryDefinitions).forEach((queryKey) => {
          ...
          SyncedCron.add({
            ...
            schedule (parser) {
              ...
              debug && logger.info('IntegrationAgent.updateQueryJobs setting schedule:', jobKey, '"', parserText, '"');
  ...
  executeQuery (queryKey, deepSync) {
    debug && logger.info('IntegrationAgent.executeQuery:', this.integration._id, queryKey, deepSync);
    let self      = this,
    ...
    if (self.serviceProvider.isHealthy()) {
      try {
        ...
        if (deepSync) {
          logger.info('IntegrationAgent.executeQuery running deep sync:', this.integration._id, queryKey);
        }
        ...
        if (importResult.items && importResult.items.length) {
          debug && logger.info('IntegrationAgent.executeQuery returned:', self.integration._id, queryKey, importResult.items.length);
          ...
          logger.error('IntegrationAgent.executeQuery item processing failed:', importResult.failures.length);
        ...
        logger.error('IntegrationAgent.executeQuery failed:', queryKey, self.integration, e);
    ...
    logger.info('IntegrationAgent.executeQuery complete:', this.integration._id, queryKey, deepSync, Date.now() - startTime);
  }
  ...
  executeAllQueries (deepSync) {
    debug && logger.info('IntegrationAgent.executeAllQueries:', this.integration._id);
    let self             = this,
        queryDefinitions = self.serviceProvider.getQueryDefinitions();
   ...
   */
  reprocessItems () {
    debug && logger.info('IntegrationAgent.reprocessItems:', this.integration._id);
    let self           = this,
    ...
    ImportedItems.find({ integrationId: self.integration._id }).forEach((item, i) => {
      i % 50 === 0 && logger.info('IntegrationAgent.reprocessItems reprocessing item', i);
      ...
      if (reimportResult.success) {
        try {
          ...
          logger.error('IntegrationAgent.reprocessItems storeImportedItem failed:', e);
          throw new Meteor.Error(e.toString())
        }
      } else {
        logger.error('IntegrationAgent.reprocessItems importItem failed:', item.identifier, reimportResult.error);
      }
    });
    
    debug && logger.info('IntegrationAgent.reprocessItems complete:', self.integration._id);
  }
   ...
   */
  destroy () {
    logger.info('IntegrationAgent.destroy:', this.integration._id);
    let self = this;
