...
export class IntegrationServiceProvider {
   ...
   */
  constructor (server) {
    logger.info('Creating new IntegrationServiceProvider:', server._id, server.title);
    let self     = this;
    self.agents  = {};
    ...
    Meteor.defer(() => {
      self.observer = IntegrationServers.find({ _id: self.server._id }).observe({
        changed (newDoc, oldDoc) {
          debug && logger.info('IntegrationServiceProvider.observer.changed:', newDoc._id, newDoc.title);
    ...
    self.integrationObserver = Integrations.find({ serverId: self.server._id }).observe({
      added (integration) {
        debug && logger.info('IntegrationServiceProvider.integrationObserver.added:', integration._id);
        Meteor.defer(() => {
          self.createIntegrationAgent(integration);
        });
      },
      removed (integration) {
        logger.info('IntegrationServiceProvider.integrationObserver.removed:', integration._id);
        Meteor.defer(() => {
   ...
   */
  integrationCallMap () {
    debug && logger.info('IntegrationServiceProvider.integrationCallMap:', this.server._id, this.server.title);
    let self = this;
   ...
   */
  getCachedStatusList () {
    debug && logger.info('IntegrationServiceProvider.getCachedStatusList:', this.server._id, this.server.title);
    let self = this;
  ...
  updateHealthCheckJob () {
    debug && logger.info('IntegrationServiceProvider.updateHealthCheckJob:', this.server._id, this.server.title);
    let self = this;
    ...
    if (Clustering.isMaster()) {
      ...
      SyncedCron.add({
        ...
        schedule (parser) {
          ...
          debug && logger.info('IntegrationServiceProvider.updateHealthCheckJob setting schedule:', self.server._id, self.server.title, "'", parserText, "'");
  ...
  updateCacheServiceJob () {
    debug && logger.info('IntegrationServiceProvider.updateCacheServiceJob:', this.server._id, this.server.title);
    let self = this;
    ...
    if (Clustering.isMaster()) {
      ...
      SyncedCron.add({
        ...
        schedule (parser) {
          ...
          debug && logger.info('IntegrationServiceProvider.updateCacheServiceJob setting schedule:', self.server._id, self.server.title, "'", parserText, "'");
          return parser.text(parserText);
        },
        job () {
          self.updateCachedData();
   ...
   */
  checkHealth () {
    debug && logger.info('IntegrationServiceProvider.checkHealth:', this.server._id, this.server.title);
    let self    = this,
        details = {},
        healthy = true;
    ...
    if (self.url && self.url.hostname) {
      if (self.server.isAuthenticated === true) {
        // Ping the server
        debug && logger.info('IntegrationServiceProvider.checkHealth pinging server:', self.url.hostname, self.url.port);
        try {
          let status = Ping.host(self.url.hostname, 3);
          healthy    = status.online === true;
          if (!healthy) {
            debug && logger.info('IntegrationServiceProvider.checkHealth ping status:', status);
            details.message = 'Server did not respond to ping requests';
          }
        } catch (e) {
          logger.error('IntegrationServiceProvider.checkHealth ping failed:', self.url.hostname, e);
        ...
        if (healthy) {
          ...
          if (!healthy) {
            debug && logger.info('IntegrationServiceProvider.checkHealth auth result:', authResult);
        ...
        healthy         = false;
        details.message = 'Server is not authenticated';
        debug && logger.info('IntegrationServiceProvider.checkHealth not authenticated:', self.url.hostname);
      }
  ...
  authenticate (username, password) {
    debug && logger.info('IntegrationServiceProvider.authenticate:', this.server._id, this.server.title, username);
    let self       = this,
   ...
   */
  unAuthenticate () {
    debug && logger.info('IntegrationServiceProvider.unAuthenticate:', this.server._id, this.server.title);
    let self   = this,
   ...
   */
  reAuthenticate () {
    debug && logger.info('IntegrationServiceProvider.reAuthenticate:', this.server._id, this.server.title);
    let self       = this,
   ...
   */
  storeAuthData (data) {
    debug && logger.info('IntegrationServiceProvider.storeAuthData:', this.server._id, this.server.title);
    let self = this;
   ...
   */
  clearAuthData () {
    logger.warn('IntegrationServiceProvider.clearAuthData:', this.server._id, this.server.title);
    let self = this;
   ...
   */
  fetchData (request) {
    debug && logger.info('IntegrationServiceProvider.fetchData:', this.server._id, this.server.title);
    let self = this;
   ...
   */
  loadCachedData () {
    debug && logger.info('IntegrationServiceProvider.loadCachedData:', this.server._id, this.server.title);
    let self = this;
   ...
   */
  updateCachedData () {
    debug && logger.info('IntegrationServiceProvider.updateCachedData:', this.server._id, this.server.title);
    let self = this;
  ...
  storeCachedItem (key, value) {
    debug && logger.info('IntegrationServiceProvider.storeCachedItem:', this.server._id, this.server.title, key);
    let self = this;
    ...
    if (value && _.isArray(value)) {
      ...
      logger.error('IntegrationServiceProvider.storeCachedItem passed an invalid value for key:', key, value);
    }
  }
   ...
   */
  getCachedData (key) {
    debug && logger.info('IntegrationServiceProvider.getCachedData:', this.server._id, this.server.title, key);
    let self = this;
  ...
  importItem (importFunction, processedItem, projectId) {
    trace && logger.info('IntegrationServiceProvider.importItem:', this.server._id, this.server.title);
    let self = this;
  ...
  importItems (importFunction, processedItems, projectId) {
    debug && logger.info('IntegrationServiceProvider.importItems:', this.server._id, this.server.title, processedItems.length);
    let self   = this,
        result = {
          items   : [],
          failures: []
    ...
    debug && logger.info('IntegrationServiceProvider.importItems complete:', this.server._id, this.server.title, processedItems.length);
  ...
  importItemsFromQuery (importFunction, query, limit) {
    debug && logger.info('IntegrationServiceProvider.importItemsFromQuery:', this.server._id, this.server.title);
    let self           = this,
  ...
  testImportFunction (importFunction, identifier) {
    debug && logger.info('IntegrationServiceProvider.testImportFunction:', this.server._id, this.server.title);
    let self          = this,
  ...
  testIntegration (integration, details) {
    debug && logger.info('IntegrationServiceProvider.testIntegration:', this.server._id, this.server.title, integration._id);
    let self = this;
  ...
  postProcessItem (rawItem) {
    trace && logger.info('IntegrationServiceProvider.postProcessItem:', this.server._id, this.server.title);
    let self          = this,
  ...
  processItemForContributors (processedItem) {
    trace && logger.info('IntegrationServiceProvider.processItemForContributors:', this.server._id, this.server.title);
    let self = this;
  ...
  storeImportedItem (integrationId, projectId, itemType, item) {
    trace && logger.info('IntegrationServiceProvider.storeImportedItem:', this.server._id, this.server.title, item && item.identifier);
    let self = this;
    ...
    if (item.identifier.length) {
      ...
      logger.error('IntegrationServiceProvider.storeImportedItem failed:', this.server._id, this.server.title, item && item.identifier);
  ...
  findOrCreateContributor (key, email, name, rawData) {
    trace && logger.info('IntegrationServiceProvider.findOrCreateContributor:', this.server._id, this.server.title, key);
    let self = this;
    ...
    if (!_.isString(key) && key.length && _.isString(email) && email.length && _.isString(name) && name.length) {
      logger.error('IntegrationServiceProvider.findOrCreateContributor passed invalid data:', key, email, name);
      return rawData;
    }
    ...
    if (self.cache.contributors[ key ]) {
      ...
      if (contributor) {
        ...
        if (contributor) {
          ...
          debug && logger.info('IntegrationServiceProvider.findOrCreateContributor created contributor:', key);
        }
      }
      return self.cache.contributors[ key ]
    }
  ...
  createIntegrationAgent (integration) {
    logger.info('IntegrationServiceProvider.createIntegrationAgent:', integration._id);
    let self = this;
    ...
    if (agent == null) {
      ...
      logger.error('IntegrationServiceProvider.createIntegrationAgent integration already exists:', integration._id);
    }
  ...
  setIntegrationAgent (integration, agent) {
    debug && logger.info('IntegrationServiceProvider.setIntegrationAgent:', integration._id);
  ...
  getIntegrationAgent (integration) {
    debug && logger.info('IntegrationServiceProvider.getIntegrationAgent:', integration._id);
  ...
  updateIntegrationAgent (newDoc, oldDoc) {
    logger.info('IntegrationServiceProvider.updateIntegrationAgent:', newDoc._id);
    let self = this;
  ...
  destroyIntegrationAgent (integration) {
    logger.info('IntegrationServiceProvider.destroyIntegrationAgent:', integration._id);
    let self  = this,
   ...
   */
  parseBaseUrl () {
    debug && logger.info('IntegrationServiceProvider.parseBaseUrl:', this.server._id, this.server.title);
    let self = this;
    ...
    try {
      ...
      logger.error('IntegrationServiceProvider could not parse baseUrl:', self.server.baseUrl, e);
    }
  }
   ...
   */
  destroy () {
    logger.info('IntegrationServiceProvider.destroy:', this.server._id, this.server.title);
    let self = this;
